import { addProfile } from './services/clientService.js';
import { createInlineError, withButtonLoading, preventDuplicate } from './utils/asyncUtils.js';

export function openAddProfileModal(clientId, onSuccess) {
  ...
  modalContent.innerHTML = `
    <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
      <h2 class="text-xl font-bold text-gray-900">Add Purchase Profile</h2>
      <button id="close-add-profile" class="text-gray-400 hover:text-gray-600 transition-colors p-1 rounded-lg hover:bg-gray-100">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
      </button>
    </div>
    <form id="add-profile-form" class="p-6 space-y-4">
      ...
      <div class="flex justify-end space-x-3 pt-4 border-t border-gray-100">
        <button type="button" id="cancel-add-profile" class="px-4 py-2 text-sm font-semibold text-gray-600 hover:text-gray-800 transition-colors">Cancel</button>
        <button type="submit" id="submit-add-profile" class="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition-all shadow-md shadow-blue-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2">
  ...
  const form = modalContent.querySelector('#add-profile-form');
  ...
  const handleSubmit = preventDuplicate(async () => {
    ...
    const newProfile = await addProfile(clientId, profileData);
